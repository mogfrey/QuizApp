...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView section = text("Total funds", 24, MUTED, false);
        page.addView(section);
        addSpace(page, 14);

        TextView total = text("40,000.00 USD", 48, TEXT, false);
        ...
        addSpace(page, 34);
        page.addView(text("Latest transactions", 24, MUTED, false));
        addSpace(page, 22);
        ...
        addSpace(page, 30);
        TextView footer = text("Goal visualization • Static offline dashboard", 13, Color.rgb(110,110,110), false);
    ...
    private View topHeader() {
        ...
        TextView name = text("Geoffrey Mogendi", 31, TEXT, true);
        LinearLayout.LayoutParams nameLp = new LinearLayout.LayoutParams(0, dp(52), 1f);
        name.setGravity(Gravity.CENTER_VERTICAL);
        row.addView(name, nameLp);

        TextView profile = text("♙", 37, TEXT, false);
        profile.setGravity(Gravity.CENTER);
        row.addView(profile, new LinearLayout.LayoutParams(dp(52), dp(52)));

        FrameLayout bellWrap = new FrameLayout(this);
        TextView bell = text("♢", 36, TEXT, false);
        bell.setGravity(Gravity.CENTER);
        bellWrap.addView(bell, new FrameLayout.LayoutParams(dp(56), dp(56), Gravity.CENTER));

        TextView badge = text("25", 16, Color.WHITE, true);
        badge.setGravity(Gravity.CENTER);
        GradientDrawable badgeBg = rounded(Color.rgb(239,92,54), dp(22), 0, 0);
        badge.setBackground(badgeBg);
        FrameLayout.LayoutParams badgeLp = new FrameLayout.LayoutParams(dp(42), dp(34), Gravity.TOP | Gravity.END);
        badgeLp.setMargins(0,0,0,0);
        bellWrap.addView(badge, badgeLp);

        row.addView(bellWrap, new LinearLayout.LayoutParams(dp(68), dp(58)));
        return row;
    }
    ...
    private View currencyCard(String flag, String label, String amount) {
        ...
        TextView title = text(label, 26, TEXT, false);
        card.addView(title);
        addSpace(card, 18);
        ...
        TextView flagView = text(flag, 38, TEXT, false);
        ...
        TextView value = text(amount, 38, TEXT, true);
        valueRow.addView(value, new LinearLayout.LayoutParams(
    ...
    private View datePill(String date) {
        TextView pill = text(date, 24, TEXT, true);
    ...
    private View navItem(String symbol, String label, boolean active) {
        ...
        TextView icon = text(symbol, 31, active ? Color.rgb(64,207,204) : TEXT, false);
        icon.setGravity(Gravity.CENTER);
        item.addView(icon, new LinearLayout.LayoutParams(-1, dp(42)));

        TextView name = text(label, 17, TEXT, active);
    ...
    private TextView text(String value, int sp, int color, boolean bold) {
        ...
        else v.setTypeface(android.graphics.Typeface.DEFAULT, android.graphics.Typeface.NORMAL);
